...
@section('breadcrumb')
    <div class="d-flex align-items-baseline flex-wrap mr-5">
        ...
        <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
            ...
            <li class="breadcrumb-item">
                <apan class="text-muted">{{__('words.edit_service')}}</apan>
            </li>
        </ul>
        <!--end::Breadcrumb-->
    </div>
                    ...
                    <div class="tab-pane fade show @if ($key == 0) active @endif" id="{{ $locale }}"
                        ...
                        <div class="col form-group">
                            <label>{{__('words.short_description')}}({{__('words.locale-' . $locale)}})<span
                                    class="text-danger">*</span></label>
                            <textarea
                                class="form-control ckeditor @error($locale . '.short_description') is-invalid @enderror "
                                type="text"
                                name="{{ $locale . '[short_description]' }}"
                                rows="4">{{ old($locale . '.short_description',$service->translate($locale)->short_description) }} </textarea>
                        ...
                        <div class="col form-group">
                            ...
                            <textarea
                                ...
                                rows="4">{{ old($locale . '.description',$service->translate($locale)->description) }} </textarea>
    ...
    <div class="card card-custom">
        <div class="card-body">
            <div class="form-group row">
                @include('admin.components.icon',['label'=>__('words.icon'),'value'=>old('icon',$service->icon)])
            </div>

            <div class="form-group row">
                @include('admin.components.file',['label'=>__('words.image'),'value'=>old('image',$service->image),'name'=>'image','id'=>'kt_image_3','accept' =>'image/*'])

                @include('admin.components.switch',['label'=>__('words.status'),'name'=>'status','val' => old('status',$service->status)])
            </div>
