...
@section('breadcrumb')
    <div class="d-flex align-items-baseline flex-wrap mr-5">
        ...
        <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
            ...
            <li class="breadcrumb-item">
                <apan class="text-muted">{{__('words.show_courses')}}</apan>
            </li>
        </ul>
        <!--end::Breadcrumb-->
    </div>
                ...
                <table class="table table-separate table-head-custom table-checkable" id="kt_datatable">
                    ...
                    @foreach($courses as $key => $course)
                        <tr>
                            ...
                            <td>{{$key+1}}</td>
                            <td>{{$course->title}}</td>
                            <td>{{createdAtFormat($course->created_at)}}</td>
                            <td>{{createdAtFormat($course->created_at) == updatedAtFormat($course->updated_at) ? '--' : updatedAtFormat($course->updated_at)}}</td>
                            <td nowrap="nowrap">
                                @include('admin.components.form-controls', ['name'=>'courses', 'value'=>$course,'role'=>'courses'])
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
